  ...
  return (
    <div className="w-full bg-white/70 backdrop-blur-xl sticky top-0 z-50 border-b border-slate-100/50">
      <div className="max-w-7xl mx-auto px-6 h-20 flex justify-between items-center">
        {/* Logo */}
        <div className="flex items-center gap-3 cursor-pointer group" onClick={() => navigate("/Facultydash")}>
          <div className="relative">
            <div className="absolute inset-0 bg-[#3B021F]/5 rounded-xl blur-lg group-hover:blur-xl transition-all"></div>
            <img src={cmslogo} alt="University Logo" className="relative w-10 h-10 object-contain" onError={(e) => { e.target.style.display = 'none'; }} />
          </div>
          <span className="text-xl font-black tracking-tight text-[#3B021F] hidden sm:block uppercase">Faculty</span>
        </div>
            ...
            return (
              <button
                ...
                className={`px-6 py-2 rounded-xl text-xs font-black uppercase tracking-[0.1em] transition-all duration-300 ${isActive
                  ? "bg-[#3B021F] text-white shadow-sm"
        ...
        <button
          ...
          className="ml-4 flex items-center gap-2.5 px-6 py-3 bg-white border border-slate-200 hover:border-[#3B021F] hover:text-[#3B021F] rounded-xl text-xs font-black uppercase tracking-[0.1em] transition-all shadow-sm active:translate-y-0"
        >
